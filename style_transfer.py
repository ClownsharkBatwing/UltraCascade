...
class StyleCascadeC_Attn(Stylizer):
    def __init__(self, mode):
        super().__init__()
        self.attn_norm = [0.0]
        self.q_proj = [0.0]
...
class StyleCascadeC_BaseBlock(Stylizer):
    def __init__(self, mode="passthrough"):

        self.res_block      = StyleUNet_ResBlock(mode)    # TODO: MAKE INTO CASCADE
        self.timestep_block = StyleUNet_ResBlock(mode)
        self.attn_block     = StyleCascadeC_Attn(mode)
    ...
    def set_len(self, h_len, w_len, img_slice, txt_slice, HEADS):
        ...
        self.res_block     .set_len(h_len, w_len, img_slice, txt_slice, HEADS)
        self.timestep_block.set_len(h_len, w_len, img_slice, txt_slice, HEADS)
        self.attn_block    .set_len(h_len, w_len, img_slice, txt_slice, HEADS)
        
        for i, mask in enumerate(self.mask):
            if mask is not None and mask.ndim > 1:
                self.mask[i] = F.interpolate(mask.unsqueeze(0), size=(h_len, w_len)).flatten().to(torch.bfloat16).cuda()
            self.res_block.mask      = self.mask
            self.timestep_block.mask = self.mask
            self.attn_block.mask     = self.mask
            
        for i, mask in enumerate(self.attn_mask):
            if mask is not None and mask.ndim > 1:
                self.attn_mask[i] = F.interpolate(mask.unsqueeze(0), size=(h_len, w_len)).flatten().to(torch.bfloat16).cuda()
            self.attn_block.mask = self.attn_mask     
...
class StyleCascadeC_InputBlock(StyleCascadeC_BaseBlock):
    def __init__(self, mode="passthrough"):
        super().__init__(mode)    

class StyleCascadeC_OutputBlock(StyleCascadeC_BaseBlock):
    def __init__(self, mode="passthrough"):
        super().__init__(mode)



class StyleCascadeC_Model(Style_Model):

    def __init__(self, dtype=torch.float64, device=torch.device("cuda")):
        super().__init__(dtype, device)
        self.input_blocks  = [StyleCascadeC_InputBlock()  for _ in range(100)]
        self.middle_blocks  = [StyleCascadeC_InputBlock()  for _ in range(100)]  # TODO: make real middle blocks
        self.output_blocks = [StyleCascadeC_OutputBlock() for _ in range(100)]
